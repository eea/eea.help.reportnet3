...
Pull requests run the same build, but do not deploy. Two checks run:

- `mkdocs build --strict` **fails** on a broken link written in a page, or a page missing from the navigation.
- `tools/check_built_site.py` **fails** on faults that the strict build cannot see: navigation entries that do not resolve, and redirects that point at themselves. Both have broken this site in the past, hence the check.

If your pull request shows a red cross, the site would have been broken — read the log, fix it, push again.
...
One trap to know about. MkDocs serves directory URLs, so `webforms.md` and `webforms/index.md` are both published at `/webforms/`. A redirect whose source resolves to the same URL as an existing page **overwrites that page**, because the redirect stubs are written after the build. If it collides with its own target you get a page that reloads forever; if it collides with a different page, that page vanishes. The build check catches this, but it is easier to not write it: a redirect is only ever needed when the old URL no longer has a page of its own.
...
| `tools/make_dark_logo.py` | Regenerates the footer's EEA logo variant. The supplied logo is black lettering on transparency and vanishes on the dark footer, so this whitens the lettering while leaving the starburst in its brand colours. Re-run it if the EEA updates its logo. |
